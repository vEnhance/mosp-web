...
import mospweb.settings as settings
...
class UserMultipleChoiceField(forms.ModelMultipleChoiceField):
    def __init__(self, *args, **kwargs):
        ...
        if "queryset" not in kwargs:
            kwargs["queryset"] = User.objects.filter(is_staff=True).order_by(*orderings)
...
@staff_member_required  # noqa: C901
...
def annotate_users_helper(user_list, annotation_kwargs):
    id_dict = dict()
    for my_user in User.objects.filter(is_staff=True).annotate(**annotation_kwargs):
...
@staff_member_required
def users(request):
    users = list(User.objects.filter(is_staff=True).select_related("profile"))
...
def users_statuses(request):
    ...
    users = User.objects.filter(is_staff=True).select_related("profile").annotate(**annotation_kwargs)
